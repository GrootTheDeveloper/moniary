...
void main() {
  ...
  test('mock repository paginates group and community cursors', () async {
    final repository = GroupMockRepository(
      GroupMockDataSource(currentUserId: 'mock-user-id'),
    );

    final groups = await repository.fetchGroupsPage(limit: 1);
    expect(groups.items, hasLength(1));
    expect(groups.hasMore, isTrue);

    final firstPage = await repository.fetchCommunityFeedPage(
      groupId: 'mock-group-dalat',
      limit: 2,
    );
    ...
    final secondPage = await repository.fetchCommunityFeedPage(
      groupId: 'mock-group-dalat',
      limit: 2,
    ...
    expect(
      secondPage.items
          .map((item) => item.id)
          .toSet()
          .intersection(firstPage.items.map((item) => item.id).toSet()),
      isEmpty,
    );
  });
