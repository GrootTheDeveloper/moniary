...
void main() {
  ...
  test('only the owner can promote and demote group members', () async {
    final owner = GroupMockDataSource(currentUserId: 'owner');
    final member = GroupMockDataSource(currentUserId: 'member');
    final groupId = await owner.createGroup(name: 'Role permissions');
    final invite = await owner.createInviteLink(groupId);
    await member.acceptInvite(tokenFrom(invite));

    await owner.updateMemberRole(
      groupId: groupId,
      userId: 'member',
  ...
  test('savings contributions cannot exceed the remaining target', () async {
    final owner = GroupMockDataSource(currentUserId: 'owner');
    final member = GroupMockDataSource(currentUserId: 'member');
    final groupId = await owner.createGroup(name: 'Savings challenge');
    final invite = await owner.createInviteLink(groupId);
    await member.acceptInvite(tokenFrom(invite));
    final now = DateTime.now();
    final challengeId = await owner.createSavingsChallenge(
      groupId: groupId,
    ...
    await member.addSavingsContribution(challengeId: challengeId, amount: 80);
    expect(
      () => member.addSavingsContribution(challengeId: challengeId, amount: 30),
      throwsA(
        isA<AppException>().having(
          (error) => error.code,
          'code',
          'CHALLENGE_CONTRIBUTION_EXCEEDS_REMAINING',
        ),
      ),
    );

    await member.addSavingsContribution(challengeId: challengeId, amount: 20);
    final completed = (await member.fetchSavingsChallenges(groupId)).single;
    expect(completed.totalContributed, 100);
    expect(completed.isActive, isFalse);
    expect(
      () => member.addSavingsContribution(challengeId: challengeId, amount: 1),
      throwsA(
        isA<AppException>().having(
          (error) => error.code,
          'code',
