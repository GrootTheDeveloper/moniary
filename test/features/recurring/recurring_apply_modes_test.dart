...
void main() {
  ...
  test('materialized transactions are linked to their rule', () async {
    ...
    expect(
      TransactionRepository.mockTransactions
          .every((t) => t.recurringTransactionId == id),
      isTrue,
    );
  });

  test('updateGeneratedTransactions rewrites shared fields, keeps dates',
      () async {
    final id = await seedDailyRule();
    await service.run();
  ...
  test('deleteGeneratedTransactions removes only that rule\'s transactions',
      () async {
    final id = await seedDailyRule();
    await service.run();
