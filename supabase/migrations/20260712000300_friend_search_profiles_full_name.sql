...
as $$
    ...
    where auth.uid() is not null
      ...
      and (
          ...
          or lower(coalesce(p.full_name, '')) like '%' || n.raw_query || '%'
          or (
    ...
    order by
        case
            when lower(coalesce(p.username, '')) = n.username_query then 0
            when lower(coalesce(p.username, '')) like n.username_query || '%' then 1
            else 2
        end,
        lower(coalesce(p.username, p.full_name, p.id::text))
    limit 20;
$$;
