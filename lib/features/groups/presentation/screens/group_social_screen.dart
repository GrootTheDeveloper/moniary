...
class _SummaryTab extends ConsumerWidget {
    ...
    return statsAsync.when(
      ...
      data: (stats) => ListView(
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                if (stats.topCategoryName != null)
                  Text(
                    'Top chi: ${stats.topCategoryName} (${formatVnd(stats.topCategoryAmount)})',
                  ),
              ],
            ),
          ),
          const SizedBox(height: 18),
          FilledButton.icon(
            onPressed: () => Share.share(_summaryText(stats)),
  ...
  String _summaryText(GroupMonthlyStats stats) {
    return 'Moniary - Tổng kết nhóm ${DateFormat('MM/yyyy').format(stats.month)}\n'
        'Tổng chi: ${formatVnd(stats.totalSpent)}\n'
            ...
            ...items.map(
              (item) => Card(
                ...
                child: SwitchListTile(
                  ...
                  secondary: Text(formatVnd(item.amount)),
                ),
              ),
            ),
        ],
