...
class GroupMockRepository implements GroupRepository {
  ...
  Future<SpendingGroupPage> fetchGroupsPage({
    ...
    final groups = [...await _source.fetchGroups()]
      ..sort((left, right) {
        final updated = right.updatedAt.compareTo(left.updatedAt);
        return updated != 0 ? updated : right.id.compareTo(left.id);
      });
    final filtered = beforeUpdatedAt == null
        ? groups
        : groups
              .where((group) {
                final updated = group.updatedAt.compareTo(beforeUpdatedAt);
                if (updated != 0) return updated < 0;
                return beforeId == null || group.id.compareTo(beforeId) < 0;
              })
              .toList(growable: false);
    final items = filtered.take(pageSize).toList(growable: false);
    return SpendingGroupPage(
      items: items,
  ...
  Future<GroupCommunityPage> fetchCommunityFeedPage({
    ...
    final pageSize = limit.clamp(1, 50);
    final posts = await _source.fetchCommunityPosts(
      groupId: groupId,
      offset: 0,
      limit: 1000,
    );
    final polls = await _source.fetchPolls(groupId);
    final challenges = await _source.fetchSavingsChallenges(groupId);
    final all = <GroupCommunityFeedItem>[
      for (final post in posts)
        GroupCommunityFeedItem(
          id: 'post-${post.id}',
          sourceId: post.id,
          groupId: groupId,
    ...
    ]..sort(_compareCommunityItems);
    final filtered = before == null
        ? all
        : all.where((item) => _isCommunityItemAfter(item, before)).toList();
    final items = filtered.take(pageSize).toList(growable: false);
    ...
    return GroupCommunityPage(
      ...
      nextCursor: last == null
          ...
          : GroupCommunityCursor(
              ...
              itemType: last.type.name,
              itemId: last.sourceId,
            ),
    );
  }
  ...
  Future<GroupCommunityCommentsPage> fetchCommunityCommentsPage({
    ...
    final pageSize = limit.clamp(1, 50);
    GroupCommunityPost? matched;
    for (final group in await _source.fetchGroups()) {
      final posts = await _source.fetchCommunityPosts(
        groupId: group.id,
        offset: 0,
        limit: 1000,
      );
      matched = posts.where((post) => post.id == postId).firstOrNull;
      if (matched != null) break;
    }
    final comments = [...?matched?.comments]
      ..sort((left, right) {
    ...
    final filtered = beforeCreatedAt == null
        ...
        : comments
              .where((comment) {
                final created = comment.createdAt.compareTo(beforeCreatedAt);
                if (created != 0) return created < 0;
                return beforeId == null || comment.id.compareTo(beforeId) < 0;
              })
              .toList(growable: false);
    final items = filtered.take(pageSize).toList(growable: false);
  ...
  Future<String> createCommunityPost({
    ...
    onMediaUploadProgress?.call(0, media.length);
    final postId = await _source.createCommunityPost(
      groupId: groupId,
      type: type,
      content: content,
      media: media,
    );
    onMediaUploadProgress?.call(media.length, media.length);
    return postId;
  }

  static int _compareCommunityItems(
    GroupCommunityFeedItem left,
    GroupCommunityFeedItem right,
  ) {
    final created = right.createdAt.compareTo(left.createdAt);
    if (created != 0) return created;
    final type = right.type.name.compareTo(left.type.name);
    return type != 0 ? type : right.sourceId.compareTo(left.sourceId);
  }

  static bool _isCommunityItemAfter(
    GroupCommunityFeedItem item,
    GroupCommunityCursor cursor,
  ) {
    final created = item.createdAt.compareTo(cursor.createdAt);
    if (created != 0) return created < 0;
    final type = item.type.name.compareTo(cursor.itemType);
    if (type != 0) return type < 0;
    return item.sourceId.compareTo(cursor.itemId) < 0;
  }
