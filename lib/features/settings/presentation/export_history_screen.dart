      ...
      body: SafeArea(
        child: historyAsync.when(
          data: (items) {
            if (items.isEmpty) {
              return Center(
                child: Text('Chưa có file export nào.', style: Theme.of(context).textTheme.bodyLarge),
              );
            }
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: Text(entry.format, style: const TextStyle(color: AppTheme.mint, fontSize: 11)),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(entry.dataTypes.join(', '), style: Theme.of(context).textTheme.bodyMedium),
                const SizedBox(height: 2),
                Text(entry.dateRange, style: Theme.of(context).textTheme.bodyMedium),
              ],
            ),
          ),
          IconButton(
            onPressed: () async {
              final opened = await ref.read(fileActionServiceProvider).open(File(entry.path));
              if (!context.mounted) {
                return;
              }
              if (!opened) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Không mở được file này. File có thể đã bị xóa.')),
                );
              }
            },
