...
| Debt calculation backend | Medium | Group and Debt features are implemented locally with mock data and pending Supabase tables. | Backend tables `expense_groups`, `group_members`, `group_expenses`, `group_expense_splits` must be verified when connecting Groups to Supabase. |
| Localization Architecture Violations (Resolved) | Low | Previously, `AppLocalizations` and UI strings were hardcoded in Domain/Data layers. | Resolved by using `BuildContext` extensions in the `shared/utils` presentation layer. Ensure future models do not contain `.label` properties. |
