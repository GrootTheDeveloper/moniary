...
| OCR backend requires local Ollama | Medium | `backend/ocr/` calls a locally hosted vision model and can take up to 120 seconds. | Deploy FastAPI/Ollama on a reachable host for shared or production use; configure `OCR_API_URL`. |
