...
| OCR deployment parity | High | The authenticated OCR backend can optionally call Gemini and has no mock fallback; source cannot prove the deployed container and environment match. | Provision Supabase/Gemini settings, deploy the secured FastAPI image, verify unauthenticated extraction returns 401, then monitor OCR/LLM latency and fallback rate. |
