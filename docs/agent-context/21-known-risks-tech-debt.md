...
| OCR requires reachable external services | Medium | Flutter calls authenticated `FastApiOcrService`; OCR requires Supabase Auth and optionally calls Gemini, with no mock result fallback. | Deploy FastAPI/Tesseract over HTTPS, provision its Supabase/Gemini environment, verify unauthenticated extraction returns 401, and monitor OCR/LLM latency plus fallback rate. |
